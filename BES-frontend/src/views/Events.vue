...
const filtered = computed(() => {
  ...
  return base
    .filter(e => e.folderName.toLowerCase().includes(search.value.toLowerCase()))
    .sort((a, b) => {
      const aInDb = dbEvents.value.some(db => db.name === a.folderName) ? 0 : 1
      const bInDb = dbEvents.value.some(db => db.name === b.folderName) ? 0 : 1
      return aInDb - bInDb
    })
})

const isInDatabase = (folderName) => dbEvents.value.some(db => db.name === folderName)

const activeEventName = computed(() => authStore.activeEvent?.name)

function setActiveAndStay(folderName) {
...
<template>
  <div class="page-container relative">
    ...
    <LoadingOverlay v-if="loading">Loading events…</LoadingOverlay>
    ...
    >
      <EventCard
        ...
        :isAdmin="isAdmin"
        :inDatabase="isInDatabase(event.folderName)"
        :isActive="activeEventName === event.folderName"
        @toggle="toggleExpanded(event.folderID)"
        @onSetup="goToEventDetails(event.folderName, event.folderID)"
        @onDetails="goToEventDetails(event.folderName, event.folderID)"
        @onSetActive="setActiveAndStay(event.folderName)"
      />
    </div>
